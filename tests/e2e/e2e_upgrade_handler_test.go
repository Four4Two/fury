...
func (suite *IntegrationTestSuite) TestUpgradeHandler() {
	...
	suite.Run("x/evmutil consensus version 1 -> 2", func() {
		before, err := suite.Fury.Upgrade.ModuleVersions(
			beforeUpgradeCtx,
		...
		after, err := suite.Fury.Upgrade.ModuleVersions(
			afterUpgradeCtx,
			&upgradetypes.QueryModuleVersionsRequest{
				ModuleName: evmutiltypes.ModuleName,
			},
	...
	suite.Run("x/evmutil AllowedCosmosDenoms updated", func() {
		before, err := suite.Fury.Evmutil.Params(beforeUpgradeCtx, &evmutiltypes.QueryParamsRequest{})
		suite.NoError(err)
		suite.Len(before.Params.AllowedCosmosDenoms, 0)

		after, err := suite.Fury.Evmutil.Params(afterUpgradeCtx, &evmutiltypes.QueryParamsRequest{})
		suite.NoError(err)
	...
	suite.Run("EIP712 signing allowed for new messages", func() {
		before, err := suite.Fury.Evm.Params(
			beforeUpgradeCtx,
		...
		after, err := suite.Fury.Evm.Params(
			afterUpgradeCtx,
			&evmtypes.QueryParamsRequest{},
		)
		suite.NoError(err)
	...
	suite.Run("stability committee ParamsChangePermission adds AllowedCosmosDenoms", func() {
		before, err := suite.Fury.Committee.Committee(
			beforeUpgradeCtx,
		...
		after, err := suite.Fury.Committee.Committee(
			afterUpgradeCtx,
			&committeetypes.QueryCommitteeRequest{
				CommitteeId: app.MainnetStabilityCommitteeId,
			},
...
func (suite *IntegrationTestSuite) getParamsChangePerm(anyComm *codectypes.Any) []committeetypes.AllowedParamsChange {
	...
	err := suite.Fury.EncodingConfig.Marshaler.UnpackAny(anyComm, &committee)
	if err != nil {
		panic(err)
	}

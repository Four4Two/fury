...
import (
	...
	"github.com/four4two/fury/client/rest"
...
func registerQueryRoutes(cliCtx client.Context, r *mux.Router) {
	...
	r.HandleFunc(fmt.Sprintf("/%s/circulatingsupplyjinxy", types.QueryPath), getCirculatingSupplyJINXYHandlerFn(cliCtx)).Methods("GET")
...
func getCirculatingSupplyJINXYHandlerFn(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		_, page, limit, err := rest.ParseHTTPArgsWithLimit(r, 0)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
		...
		route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryCirculatingSupplyJINXY)
		res, _, err := cliCtx.QueryWithData(route, bz)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return

...
func queryCirculatingSupplyJINXY() *cobra.Command {
	return &cobra.Command{
		Use:   "circulating-supply-jinxy",
		Short: "Get JINXY circulating supply",
		Long:  "Get the current circulating supply of JINXY tokens",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			// Query
			res, height, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryCirculatingSupplyJINXY), nil)
			if err != nil {
				return err
			}
			cliCtx = cliCtx.WithHeight(height)

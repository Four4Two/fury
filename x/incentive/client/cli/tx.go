...
func getCmdClaimDelegator() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-delegator --%s jinx=large --%s jinxy=small`, version.AppName, types.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-delegator --%s jinx=large,jinxy=small`, version.AppName, types.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
...
func getCmdClaimSwap() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-swap --%s jinxy=large --%s ufury=small`, version.AppName, types.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-swap --%s jinxy=large,ufury=small`, version.AppName, types.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
...
func getCmdClaimSavings() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: strings.Join([]string{
			fmt.Sprintf(`  $ %s tx %s claim-savings --%s jinxy=large --%s ufury=small`, version.AppName, types.ModuleName, multiplierFlag, multiplierFlag),
			fmt.Sprintf(`  $ %s tx %s claim-savings --%s jinxy=large,ufury=small`, version.AppName, types.ModuleName, multiplierFlag),
		}, "\n"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)

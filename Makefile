...
HTTPS_GIT := https://github.com/Four4Two/fury.git
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/four4two/fury
.PHONY: format
